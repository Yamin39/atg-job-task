...
const SignIn = ({ setUser, setIsModalOpen, setAuthPage }) => {
  const [eyeToggle, setEyeToggle] = useState(false);
  return (
    <div className="w-full md:w-fit md:min-w-[736px] min-h-[462px] md:min-h-fit relative bg-white shadow-[0px_8px_24px_0px_rgba(0,0,0,0.25)] rounded-[8px_8px_0px_0px] md:rounded-[8px_8px_8px_8px]">
      {/* close modal */}
      <button
        onClick={() => {
          setIsModalOpen(false);
          setAuthPage("Create Account");
        }}
        className="absolute top-6 md:-top-10 right-[22px] md:-right-4 size-5 md:size-6"
      >
      ...
      <div className="px-5 md:px-9 pt-[22px] md:pt-6 mb-[18px] md:pb-9">
        <div className="flex justify-between items-center gap-2 mb-6">
          <h5 className="text-lg md:text-2xl font-bold leading-[23px] md:leading-[31px]">
            <span className="hidden md:inline">Sign In</span>
            <span className="md:hidden">Welcome back!</span>
          </h5>
          <p className="hidden md:block text-[#3D3D3D] text-[13px]">
            Don’t have an account yet?{" "}
            <button onClick={() => setAuthPage("Create Account")} className="font-semibold text-[#2F6CE5]">
              Create new for free!
            </button>
          </p>
        </div>
          ...
          <div className="w-full md:w-[48.2%]">
            <form onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="my-[22px] md:my-0 flex md:block items-center justify-between gap-2">
                ...
                <button
                  onClick={() => setAuthPage("Create Account")}
            ...
            <button className="mt-[23px] text-xs font-medium leading-4 tracking-[-0.8%] block mx-auto">Forgot Password?</button>
          </div>
          <div className="hidden md:block w-[48.2%]">
